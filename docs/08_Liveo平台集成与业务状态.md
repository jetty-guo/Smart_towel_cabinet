...
## 8. 自动记账与人工处置的交界（2026-09-20）

[21章第14节](21_端到端逻辑闭环与验收矩阵.md)细化本章证据链：人工处理遗留物必须先取得柜端耐久维护接管确认；之后S3清空记录为维护证据，不能自动给原住户记借用。接管前已确认取走或先后不明，不能直接改成未交付。

柜端终态REJECTED须已有耐久禁止执行记录；非终态忙不能退预留。已接任务离线异常用localIncidentId保存，再幂等关联caseId。平台补正与自动事件共享businessResultVersion和事务裁定；人工请求带幂等键及预期版本，旧APPLIED通知不得覆盖较新的工单或业务结论。具体字段见[09章第9节](09_设备协议与联调契约.md)，验收见[23章](23_十轮端到端逻辑复核与修订.md)。以上仍为目标设计，未核验Liveo实际实现。
