...
## 8. 协议实施包的冻结补充（2026-09-19）

[22章第7—8节](22_固定模型下的系统集成审查与工程补充.md)细化本章建议契约：所有命令、许可、事件、应用确认和查询都需可校验schema、条件字段、空值语义、载荷上限、合法状态迁移、版本兼容及黄金报文。柜内总线协议与柜端MQTT分别交付；通信回调不能直接获得电机动作权。

同一任务的持久化接受、开始意图、取消封禁和恢复须在写入/动作边界验证；共享读写器、供电及维护资源须有明确互锁与并发证据。保持1.2-draft状态，未经过双方签核联调不得改成正式发布版。SI08—SI11仍未关闭。


## 9. 四项契约补充与签核输入（2026-09-20）

执行[21章第14节](21_端到端逻辑闭环与验收矩阵.md)和[23章反例](23_十轮端到端逻辑复核与修订.md)。以下是1.2-draft待签核语义增量，不宣称已有接口或兼容发布。

| 对象 | 必补语义 | 拒绝／恢复边界 |
|---|---|---|
| 维护接管 | 原任务、channelEpoch、maintenanceSessionId、授权引用、柜端耐久接管确认和证据版本 | 与正常取走判定串行；未确认接管不允许假定工作人员取出已被排除在自动记账之外 |
| 命令拒绝结果 | 原commandId、内容摘要、作用域、是否终态、是否已耐久禁止执行、原因 | 终态REJECTED不复活；非终态CHANNEL_BUSY不退预留；非法报文不得覆盖合法ID |
| 异常同步 | localIncidentId及唯一作用域、原任务／回执、物体引用、实际位置、发现时间可信度、caseId映射与同步状态 | caseId创建前可空；禁止空值伪造为已派单；重复同步不新建第二工单／第二物体 |
| ReconcileTask | reconcileRequestId、expectedBusinessResultVersion、内容摘要、证据和授权 | 幂等重放、内容冲突、业务版本冲突分开返回；与自动事件处理共用事务裁定 |
| 业务结果确认／查询 | 应用确认的appliedBusinessResultVersion；查询／通知的当前businessResultVersion及完整业务结果 | 与柜端taskStateVersion分离；较旧结果不回滚人工修订；APPLIED不等于可复用通道 |

D18签核时同步schema、空值及错误语义、版本协商、耐久数据格式、黄金报文和恢复用例；不支持本补充能力的旧固件／服务端不得按推测字段值进入对应自动流程。安全停机不依赖接口响应。T13新增LR子场景全部待联调实测。
